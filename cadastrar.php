...
if (empty($dados['nome'])) {
    $retorna = ['status' => false, 'msg' => "<div class='alert alert-danger' role='alert'>Erro:Necessário prencher o campo nome!</div>"];
} elseif (empty($dados['email'])){
    $retorna = ['status' => false, 'msg' => "<div class='alert alert-danger' role='alert'>Erro:Necessário prencher o campo e-mail!</div>"];
} elseif (empty($dados['logradouro'])){
    $retorna = ['status' => false, 'msg' => "<div class='alert alert-danger' role='alert'>Erro:Necessário prencher o campo logradouro!</div>"];
} elseif (empty($dados['numero'])){
    $retorna = ['status' => false, 'msg' => "<div class='alert alert-danger' role='alert'>Erro:Necessário prencher o campo número!</div>"];
} else {
    ...
    if($cad_usuario->rowCount()) {
        ...
        // Cadastrar no banco de dados na segunda tabela (endereço) associada ao usuário
        $query_endereco = "INSERT INTO enderecos (logradouro, numero, usuario_id) VALUES (:logradouro, :nome, :usuario_id)";
        ...
        if ($cad_endereco->rowCount()) {
            ...
            // Se o usuário foi cadastrado com sucesso, mas o endereço não, retorna mensagem de erro
